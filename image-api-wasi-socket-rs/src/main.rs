...
async fn grayscale(req: Request<Body>) -> Result<Response<Body>, Box<dyn std::error::Error + Send + Sync>> {
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, "/v1.0/state/statestore") => {
            ...
            post_url_return_str(url, POST_BODY, len).await?;
...
async fn post_url_return_str (url: hyper::Uri, post_body: &'static [u8], len: usize) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
